...
static func Lt(val) -> Dictionary:
	assert(typeof(val) in [TYPE_INT, TYPE_FLOAT], "Invalid number to less than operator")
	return {
		"$lt": val
	}

static func Lte(val) -> Dictionary:
	assert(typeof(val) in [TYPE_INT, TYPE_FLOAT], "Invalid number to less than or equal to operator")
	return {
		"$lte": val
	}

static func Gt(val) -> Dictionary:
	assert(typeof(val) in [TYPE_INT, TYPE_FLOAT], "Invalid number to greater than operator")
	return {
		"$gt": val
	}

static func Gte(val) -> Dictionary:
	assert(typeof(val) in [TYPE_INT, TYPE_FLOAT], "Invalid number to greater than or equal to operator")
	return {
		"$gte": val
	}
